...
export function createTradeControl(
  ...
  const status = async () => {
    ...
    return {
      ...
      today_realized_pnl_krw: Number(hooks?.getStrategyMetrics?.()?.today_realized_pnl_krw ?? 0),
      cumulative_realized_pnl_krw: Number(hooks?.getStrategyMetrics?.()?.cumulative_realized_pnl_krw ?? Object.values(strategyPositions).reduce((acc, p) => acc + p.realized_pnl, 0)),
      unrealized_pnl_krw: Number(hooks?.getStrategyMetrics?.()?.unrealized_pnl_krw ?? 0),
      account_net_pnl_krw: account_portfolio?.net_pnl_krw ?? 0,
      strategy_asset_pnl_krw: Number(hooks?.getStrategyMetrics?.()?.cumulative_realized_pnl_krw ?? Object.values(strategyPositions).reduce((acc, p) => acc + p.realized_pnl, 0)),
      passive_holding_value_krw: account_portfolio?.passive_holding_value_krw ?? 0,
      cost_basis_unknown_krw: account_portfolio?.cost_basis_unknown_krw ?? 0,
      account_portfolio,
      mark_prices,
      mark_prices_stale,
