 ...
 * 원본 로직은 surge-v2/surge-entry-engine.ts로 이동됨.
 */
export function evaluateSurgeEntryPipeline(input: Parameters<typeof evaluateSurgeV2Entry>[0]) {
  const decision = evaluateSurgeV2Entry(input);
  if (decision.action === "enter") {
    return { ok: true, detail: decision.detail };
  } else {
    return { ok: false, message: decision.reason, detail: decision.detail };
  }
}
